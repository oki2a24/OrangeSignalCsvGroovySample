...
println ""
println "入力"
BufferedInputStream stream = new File("input.csv").newInputStream()
List<Customer> customers = new CsvEntityManager().config(cfg).load(Customer.class).from(stream)
...
new CsvEntityManager().config(cfg).save(customers, Customer.class).to(writer)
